      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="adminOrder in adminOrders" :key="adminOrder.id">
            <td class="text-center">{{ $displayDate(adminOrder.create_at) }}</td>
            ...
            <td class="text-right">{{ $displayCurrency(adminOrder.total) }}</td>
            <td class="text-center">
              <span v-if="adminOrder.is_paid === true" class="text-success">已付款</span>
